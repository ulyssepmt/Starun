...
void CreateGameUI(struct GAME *pGame, HANDLE hConsole, HWND hwndConsole, int xConsoleWindow, int yConsoleWindow) {
    ...
    SetConsoleTextAttribute(hConsole, 12);
    printf("You lose a coin with every collision\n\n");
    SetConsoleTextAttribute(hConsole, 13);
...
int RunGame(struct  GAME *pGame) {
    ...
    while(pGame->PLAY_AGAIN) {
            ...
            if(scanf(" %c", &choice) == 1) {
                ...
                printf("Starun was coded in C by me, sk0za and finished the 21/03/2024. I learned a lot, especially about data structures. It was a great challenge !");
