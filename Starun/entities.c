...
void DrawEnd(struct GAME *pGame, HBITMAP spaceship, HWND gameWindow, HDC hDC, HDC hDCMemoire) {
...
DWORD WINAPI DrawShots(LPVOID lparam) {
    ...
    for(int i = pGame->player.y; i > -50  /*bar*/ ; i--) {

        BitBlt(hDC, xshot + 44, i, 5, 10, hDCMemoire, 0, 0, SRCCOPY); // bitmap tirs
        ...
        BitBlt(hDC, xshot + 44, i + 20, 5, 10, hDCMemoire, 0, 0, BLACKNESS); //mask shots
...
DWORD WINAPI DrawPlayer(LPVOID lparam) {
    ...
    while (ANIMATION) {
            ...
            else if(GetAsyncKeyState(0x43) != 0)  { //C
            ...
            WaitForSingleObject(AsyncShots, INFINITE);

            CloseHandle(AsyncShots);

            DrawEnd(pGame, pGame->spaceshipBitmap, gameWin, hDC, hDCMemoire);
